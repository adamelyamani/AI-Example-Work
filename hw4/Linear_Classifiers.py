...
def Logistic_Regression(X, Y, learningRate=0.01, maxIter=100):
    ...
    ####### TODO: implement logistic regression (You should be able to implement this < 10 lines)
        ...
        loss = 0
        tempArray = np.zeros((D_plus_1, 1))

        for n in range(N):

            for m in range(D_plus_1):
                tempArray[m] = X[m][n]

            p = sigmoid(np.matmul(w.transpose(), tempArray))
            loss = loss + (Y[n]-p)*tempArray

        loss = learningRate * (- loss / N)
...
def Perceptron(X, Y, learningRate=0.01, maxIter=100):
    ...
    ####### TODO: implement perceptron (You should be able to implement this < 10 lines)
            ...
            yPredict = np.sign(np.matmul(w.transpose(), tempArray))

            if yPredict != Y[n]:
                w = w + learningRate * Y[n] * tempArray
