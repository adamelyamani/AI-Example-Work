...
def linear_regression(X, Y):
    ...
    # Create a row of ones to append to X
    ones_array = np.ones(N)
    X_tilde = np.append(X, [ones_array], axis=0)

    # Take the inverse of the dot product of X_tilde and its transpose
    w1 = np.linalg.inv(np.matmul(X_tilde, X_tilde.transpose()))
    # Take the dot product of X_tilde and Y
    w2 = np.matmul(X_tilde, Y)
    # Use the two previous results to calculate w_tilde
    w_tilde = np.matmul(w1, w2)

    b = w_tilde[D]
    w = np.delete(w_tilde, D, axis=0)
